...
1. **Runner 비의존 추론을 무엇으로 돌릴 것인가**
   - (A) 소유자 맥미니의 로컬 LLM — 비용 0, 단일 장애점, 동시성 한계
   - (B) OpenAI API — 안정적, 소유자 비용 부담
   - (C) 사용자 API 키(BYOK) — 사용자 비용, 가입 마찰
2. **인증 제공자** — 현재 미설정으로 프로덕션 로그인 불가.
   Google OAuth 직접 연동 / WorkOS 유지 / legacy 1인 모드 중 선택.
   변경 시 [ADR 0009](../adr/0009-provider-auth-calendar-signing-distribution.md)를 대체하는
   새 ADR이 필요하다.
3. **자동화 실행 위치** — 현재 Runner 의존. Calendar AI와 함께 옮길지, 종류별로 나눌지.
4. **사용량 제한** — (A) 또는 (B) 선택 시 워크스페이스별 상한이 필요하다.
