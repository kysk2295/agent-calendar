  ...
  await page.addInitScript(() => {
    window.hermesDesktop = {
      getSettings: async () => ({
        ...
        hasApiToken: false,
        theme: 'default',
        authProfile: { provider: 'password', id: 'calendar-crud-user', email: 'calendar@example.test', name: 'Calendar QA' },
        uiPreferences: { notify: true, agentShare: true, weekStartMon: true },
      }),
      getHermesConnection: async () => ({ baseUrl: '', credential: '' }),
      getSessionStatus: async () => ({ signedIn: true, sessionId: 'calendar-crud-session' }),
