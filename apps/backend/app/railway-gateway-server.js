...
function readRequestBody(req, env = process.env, maxBytes) {
  return readProductionRequestBody(req, env, { maxBytes });
}
  ...
  const turnPromise = turnEnabled
    ? runRelaySessionTurn({
      ...
      payload: {
        ...
        message: question,
        requestId,
        conversationId: String(env.HERMES_WIKI_AGENT_CONVERSATION_ID || 'agent-calendar-wiki'),
      },
  ...
  try {
    bodyBuffer = ['GET', 'HEAD'].includes(method)
      ...
      : await readRequestBody(
        req,
        env,
        relaySnapshotUpload ? RELAY_SNAPSHOT_MAX_BYTES : undefined,
      );
  } catch (error) {
    if (relaySnapshotUpload && error?.code === 'PAYLOAD_TOO_LARGE') {
